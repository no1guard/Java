...
public class FilePre {
	...
	public void 메서드2() {
		// 바탕화면에 뉴폴더라는 폴더 생성하고 뉴폴더 안에 파일생성.txt
		...
		if(파일만들기.exists()) {
			...
			try {
				파일만들기.createNewFile();
				System.out.println(파일만들기.getName()+"파일이 생성되었습니다.");
			} catch (IOException e) {
				System.out.println("파일만들기에 실패했습니다.");
				e.printStackTrace();
			}
		}
		//바탕화면 경로를 설정하는 방법
	...
	public void 메서드3() {
		...
		try {
			...
			//파일 크기 확인 long length
			System.out.println(파일.length());
			//파일 마지막 수정일 확인 long lastModified
