 ...
 * 
 ...
 메서드 
 	boolean mkdir() :폴더 생성 make폴더
 	boolean mkdirs() : 경로를 지정해준 모든 폴더 생성 
 	...
 	boolean isDirectory() : 현재 File 객체가 관리하는게 파일이면 true
...
/*
 ...
 경로		│
 		...
 		└상대 경로 : 내가 위치한 폴더나 파일 기준으로 폴더를 작성해주는 것
					./  : 현재 내 폴더 위치를 나타냄
					../ : 현재 내 폴더에서 한단계 더 상위폴더로 이동하는 것을 의미 (예를들어 바탕화면에 newFF파일이 있는데 그 상위폴더는 바탕화면으로 있어서 상위폴더로가는거)
...
public class FileEx {
	public static void main(String[] args) {
		...
		if(폴더만들기.exists()) {
			...
			//폴더생성
			폴더만들기.mkdir();
			System.out.println("폴더 새로만들기 성공");
