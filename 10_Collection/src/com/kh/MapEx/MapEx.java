...
public class MapEx {
	public static void main(String[] args) {
		...
		//remover를 활용해서 삭제하기
		학생들.remove("영희");
		//영희가 사라졌는지 전체보기를 통해 다시 확인하기
		System.out.println("전체 학생들: "+학생들);
		
		//키는 유일해야함  키의 값이 중복이 될 수 없음 맨 마지막 값으로 덮어씌워짐
		학생들.put(  "철수",   80);
		
		//학생들 성적 출력하기
		//for-each 학생들에 들어있는 key를 모두 가지고와서 배열해놓는 keySet() 활용 
		for( String 이름 :  학생들.keySet()) {//학생들.ketSet() {"철수","민수"}
			//각 학생의 성적 조회 
			int 성적 = 학생들.get(이름);
			System.out.println(이름+"의 학생의 성적은 "+성적);
		}
	}
}
